...
program
  ...
  .option('-l, --loglevel [value]', 'Log level: fatal, error, warn, info, verbose, trace')
  ...
  .option('-a, --artifacts-location [path]', '[EXPERIMENTAL] Artifacts (logs, screenshots, etc) root directory.', 'artifacts')
  .option(
    '--record-logs [failing|all|none]',
    '[EXPERIMENTAL] Save logs during each test to artifacts directory. Pass "failing" to save logs of failing tests only.'
  )
  .option(
    '--take-screenshots [failing|all|none]',
  ...
  .option(
    '--record-videos [failing|all|none]',
  ...
  .option('-H, --headless', '[Android Only] Launch Emulator in headless mode. Useful when running on CI.')
  .option(
    '-w, --workers <n>',
